...
#>
#
# This file has been changed, some strings were unhidden, some were changed to different values, keep an eye out for the commented out sections
# - DeadlineEm
#
...
function MineStat {
  ...
  enum ConnStatus {
    ...
    Online = 1 # This was perviously "Success"
    # The connection attempt failed for an Offline reason.
    Offline = 0 # This was perviously "Unknown"
  ...
  [Flags()] enum SlpProtocol {
    ...
    Legacy = 2
    Beta = 1
    Offline = 0 #T his was perviously "Unknown"
  }
...
# This section was edited, removed 'hidden' from most of the values below and added it where I saw fit.
  class ServerStatus {
    ...
    hidden [string]$formatted_motd
	hidden[string]$motd
    [string]$stripped_motd = "N/A"
    [int]$current_players = -1
    [int]$max_players = -1
    [int]$latency = -1
	[string]$gamemode
    [string[]]$playerList
    hidden[SlpProtocol]$slp_protocol

    # hidden values make a better looking return when running from the console
    hidden [int]$timeout
    [string]$connection_status = [ConnStatus]::Offline
    ...
    ServerStatus($address, $port, $timeout, [SlpProtocol]$queryprotocol) {
      ...
      if ($queryprotocol.HasFlag([SlpProtocol]::Legacy) -and $this.connection_status -notin [ConnStatus]::ConnFail, [ConnStatus]::Online) {
      ...
      if ($queryprotocol.HasFlag([SlpProtocol]::Beta) -and $this.connection_status -notin [ConnStatus]::ConnFail, [ConnStatus]::Online) {
